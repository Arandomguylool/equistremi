...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'censory-superload': //Why are they seperated? ...Erm...  ¯\_(ツ)_/¯
				hasDialog = true;
					if ((PlayState.SONG.song.toLowerCase()=='censory-superlaod') && (PlayState.cutsceneSkip = true))
						hasDialog = false;
				box.frames = Paths.getSparrowAtlas('speech_bubble_talking');
				box.animation.addByPrefix('normalOpen', 'Speech Bubble Normal Open', 24, false);
				box.animation.addByPrefix('normal', 'speech bubble normal', 24, false);
			...
			case 'exterminate': //Why are they seperated? ...Erm...  ¯\_(ツ)_/¯
				hasDialog = true;
				box.frames = Paths.getSparrowAtlas('speech_bubble_talking');
				box.animation.addByPrefix('normalOpen', 'Speech Bubble Normal Open', 24, false);
				box.animation.addByPrefix('normal', 'speech bubble normal', 24, false);
		...
		if (PlayState.SONG.song.toLowerCase()=='carefree' || PlayState.SONG.song.toLowerCase()=='careless' || PlayState.SONG.song.toLowerCase()=='censory-superload' || PlayState.SONG.song.toLowerCase()=='exterminate' || PlayState.SONG.song.toLowerCase()=='cessation')
		{
			...
			if(PlayState.SONG.song.toLowerCase()=='censory-superload'){ //Defining portraits for Censory-Superload
				...
				portraitLeftALT.frames = Paths.getSparrowAtlas('ui/roboAngryPortait');
				portraitLeftALT.animation.addByPrefix('enter', 'roboAngry_potrait', 24, false);
				portraitLeftALT.updateHitbox();
				portraitLeftALT.scrollFactor.set(1);
				add(portraitLeftALT);
				portraitLeftALT.visible = false;
				portraitLeftALT.antialiasing = true;
				...
				add(portraitRightALT);
				portraitRightALT.visible = false;
				portraitRightALT.antialiasing = true;
			}
	...
	function startDialogue():Void
	{
		...
		if(PlayState.SONG.song.toLowerCase()=='exterminate')
		{
			switch (curCharacter)
		    {
				case 'kb': //Left
		...
		if(PlayState.SONG.song.toLowerCase()=='cessation')
			{
				switch (curCharacter)
				{
					case 'kb': //Left
		...
		else
		{
			switch (curCharacter)
			{
				case 'dad':
					...
					if(PlayState.SONG.song.toLowerCase()=='carefree' || PlayState.SONG.song.toLowerCase()=='careless')
						box.flipX = true;
					portraitRight.visible = false;
					portraitLeftALT.visible = false;
					portraitRightALT.visible = false;
				...
				case 'kb-angry':
					if(PlayState.SONG.song.toLowerCase()=='censory-superload')
						box.flipX = true;
					portraitRight.visible = false;
					portraitLeft.visible = false;
					if (!portraitLeftALT.visible) //No need to re-enter if already visible.
