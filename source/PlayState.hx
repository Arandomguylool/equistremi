...
class PlayState extends MusicBeatState
{
	...
	{
		#if sys
		var data:BitmapData = OpenFlAssets.getBitmapData("assets/data/" + PlayState.SONG.song.toLowerCase() + '/' + spritePath + ".png");
	...
	override public function create()
	{
		...
		if (loadRep)
			...
			#if mobile
			if (PlayState.SONG.song.toLowerCase() == 'tutorial' || PlayState.SONG.song.toLowerCase() == 'censory-superload' || PlayState.SONG.song.toLowerCase() == 'extermination' || PlayState.SONG.song.toLowerCase() == 'expurgation' || PlayState.SONG.song.toLowerCase() == 'milf')
			{
			addMobileControls(DODGE);
			}
			else 
			{
		...
		if (executeModchart) // dude I hate lua (jkjkjkjk)
			{
				...
				var modchartFileName:String = "/modchart.lua";
				if(SONG.song.toLowerCase() == 'extermination' && storyDifficulty==1)
					modchartFileName == "/modchartUNFAIR.lua";

				var result = LuaL.dostring(lua, OpenFlAssets.getText("assets/data/" + PlayState.SONG.song.toLowerCase() + modchartFileName)); // execute le file
	...
	private function keyShit():Void
	{
		...
		if(SONG.song.toLowerCase() == "extermination" || SONG.song.toLowerCase() == 'tutorial'){
			...
			if(FlxG.keys.justPressed.SPACE #if mobile || mobileControls.hitbox.buttonDodge.justPressed #end)
				trace('butttonpressed');

			if(FlxG.keys.justPressed.SPACE #if mobile || mobileControls.hitbox.buttonDodge.justPressed #end && !bfDodging && bfCanDodge){
				trace('DODGE START!');
				bfDodging = true;
				bfCanDodge = false;
