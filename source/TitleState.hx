...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		//KB is watching you...
		kb = new FlxSprite(FlxG.width * -0.4, FlxG.height * 0.035);
		kb.frames = Paths.getSparrowAtlas('kb_titlescreen');
		kb.animation.addByPrefix('idle', 'titleanimation', 24, false);
		kb.setGraphicSize(Std.int(kb.width * 0.25));
		kb.antialiasing = true;
		kb.x+=1004;
		kb.y-=542;
		add(kb);
		...
		switch (curBeat)
		{
			...
			case 8:
				if (Main.watermarks)
					...
					createCoolText(["I didn't know"]);
			case 9:
				if (Main.watermarks)
					addEVENMoreText('By');
				else
					addEVENMoreText('How to code');
			case 10:
				if (Main.watermarks)
			...
			case 12:
				...
				createCoolText(['DrkFon376 and Marshal']);
			// credTextShit.visible = true;
			case 13:
				addMoreText('With Pain in the Fingers');
			case 14:
			...
			// credTextShit.text += '\nNight';
			case 18:
				addMoreText("EXTREME"); // credTextShit.text += '\nFunkin';
			case 19:
				if (!skippedIntro){
